...
BEGIN
    ...
    WHERE
        ...
        OR (p_last_name IS NULL OR f.last_name      LIKE CONCAT(p_last_name, '%'))
        OR (p_department IS NULL OR d.department_name LIKE CONCAT(p_department, '%'))
        OR (p_institution IS NULL OR i.name            LIKE CONCAT(p_institution, '%'));
END $$
DELIMITER ;
