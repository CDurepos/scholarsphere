...
class DatabaseConnection:
    ...
    def call_procedure(self, cursor, procedure_name: str, args: tuple) -> Optional[Dict[str, Any]]:
        """
...
def find_scraped_files(output_dir: str) -> Tuple[List[str], List[str]]:
    ...
    Returns:
        Tuple of (institution_files, faculty_files)
    """
    institution_files = []
    faculty_files = []
    ...
    for filename in os.listdir(output_dir):
        ...
        if filename.endswith('_institution.json'):
            institution_files.append(filepath)
        elif filename.endswith('_faculty.jsonl'):
            faculty_files.append(filepath)
    
    return sorted(institution_files), sorted(faculty_files)
...
def insert_institution_record(record: Dict[str, Any], db: DatabaseConnection) -> Optional[str]:
    """
...
def insert_faculty_record(record: Dict[str, Any], db: DatabaseConnection, institution_id_map: Dict[str, str]) -> bool:
    """
    ...
    try:
        ...
        faculty_id = generate_faculty_id(first_name, last_name, scraped_from)
        ...
        for dept in departments:
            if dept:
                db.call_procedure(cursor, 'create_faculty_department', (faculty_id, dept))
        ...
        if conn and conn.in_transaction:
            conn.rollback()
        print(f"[ERROR] Failed to insert faculty {record.get('first_name')} {record.get('last_name')}: {str(e)}")
        return False
...
def main():
    ...
    institution_files, faculty_files = find_scraped_files(output_dir)
    ...
    for faculty_file in faculty_files:
        ...
        try:
            ...
            for i, record in enumerate(records, start=1):
                if insert_faculty_record(record, db, institution_id_map):
                    total_stats['successful'] += 1
                else:
                    total_stats['failed'] += 1
                
                if i % 10 == 0:
                    print(f"[INFO] Processed {i}/{len(records)} records from {os.path.basename(faculty_file)}")
